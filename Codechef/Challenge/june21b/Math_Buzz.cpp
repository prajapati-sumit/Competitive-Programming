...
void fn2(int cur,int l,int r){
    // cerr<<cur<<" "<<l<<" "<<r<<'\n';
    if (l == r) {
        product[cur]={ (M-a[l])%M, 1};
        return;
    }
    int mid = (l + r) >> 1;
    fn2(cur<<1,l, mid);
    fn2((cur<<1)^1,mid + 1, r);
    mult(product[cur<<1], product[(cur<<1)^1], product[cur]);

}
void buildtree(int cur,int start,int end){
    vector<int>q;
    PolyDiv(segm[cur>>1],product[cur],q,segm[cur]);
